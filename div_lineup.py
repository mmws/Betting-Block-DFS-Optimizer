...
if uploaded_file:
    try:
        ...
        # Validate and clean AvgPointsPerGame
        try:
            ...
            st.error(f"⚠️ Failed to process AvgPointsPerGame or Salary: {e}")
            raise
        ...
        if missing:
            ...
            if salary_df["Position"].isna().any() or salary_df["Name"].isna().any() or salary_df["ID"].isna().any() or \
               salary_df["TeamAbbrev"].isna().any() or salary_df["Game Info"].isna().any():
                ...
                if st.button("Generate Lineups"):
                    ...
                    for _, row in salary_df.iterrows():
                        try:
                            ...
                            last_name = name_parts[1] if len(name_parts) > 1 else "" if row["Position"] != "DST" else row["Name"]
                            player = Player(
