...
max_repeating_players = st.slider("Max repeating players per lineup", 0, len(players), 2)
...
if gen_btn:
    ...
    if final_lineups:
        st.success(f"Generated {len(final_lineups)} lineup(s)")

        # --- Prepare CSV with proper headers ---
        position_order = ["QB","RB","RB","WR","WR","WR","TE","FLEX","DST"]
        csv_rows = []

        for lineup in final_lineups:
            row = {h:"" for h in position_order+["TotalSalary","ProjectedPoints"]}
            for p in lineup:
                p_pos = p.positions[0] if p.positions else None

                # Assign to primary positions first
                if p_pos in ["QB","DST","TE"]:
                    if row[p_pos]=="":
                        row[p_pos] = f"{player_display_name(p)}({p.id})"
                elif p_pos=="RB":
                    for slot in ["RB","RB"]:
                        if row[slot]=="":
                            row[slot] = f"{player_display_name(p)}({p.id})"
                            break
                    else:
                        if row["FLEX"]=="":
                            row["FLEX"] = f"{player_display_name(p)}({p.id})"
                elif p_pos=="WR":
                    for slot in ["WR","WR","WR"]:
                        if row[slot]=="":
                            row[slot] = f"{player_display_name(p)}({p.id})"
                            break
                    else:
