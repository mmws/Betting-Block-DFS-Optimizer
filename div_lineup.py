...
def diversify_lineups_wide(
    ...
    for _, row in salary_df.iterrows():
        try:
            ...
            st.warning(f"Invalid AvgPointsPerGame for {row['Name']}: {row['AvgPointsPerGame']}. Skipping.")
            continue
...
if uploaded_file:
    try:
        ...
        try:
            ...
            st.error(f"⚠️ Failed to process AvgPointsPerGame: {e}")
            raise
        ...
        if missing:
            ...
            if salary_df["Position"].isna().any() or salary_df["Name"].isna().any() or salary_df["ID"].isna().any() or \
               salary_df["Salary"].isna().any() or salary_df["TeamAbbrev"].isna().any() or salary_df["Game Info"].isna().any():
