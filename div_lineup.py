...
max_repeating_players = st.slider("Max repeating players across lineups",1,10,3)
gen_btn = st.button("Generate lineups")

def enforce_max_repeating_players(lineups, max_repeat):
    player_counts = {}
    filtered = []
    for lineup in lineups:
        lineup_players = getattr(lineup,"players",None) or getattr(lineup,"_players",None) or list(lineup)
        repeat_count = sum(player_counts.get(p.id,0) for p in lineup_players)
        if repeat_count <= max_repeat * len(lineup_players):
...
if gen_btn:
    with st.spinner("Generating..."):
        raw_lineups = list(optimizer.optimize(n=num_lineups*2, max_exposure=max_exposure))
        lineups = enforce_max_repeating_players(raw_lineups, max_repeating_players)[:num_lineups]
    st.success(f"Generated {len(lineups)} lineup(s)")

    # --- Convert to wide format with correct headers ------------------------
    csv_headers = ["QB","RB","RB","WR","WR","WR","TE","FLEX","DST","TotalSalary","ProjectedPoints"]
    wide_rows = []

    for lineup in lineups:
        lineup_players = getattr(lineup,"players",None) or getattr(lineup,"_players",None) or list(lineup)
        row = {}
        pos_counts = {"RB":0,"WR":0}

        for p in lineup_players:
            pos = p.positions[0] if p.positions else "FLEX"
            if pos in ["RB","WR"]:
                idx = [i for i,h in enumerate(csv_headers) if h==pos][pos_counts[pos]]
                row[csv_headers[idx]] = f"{player_display_name(p)}({p.id})"
                pos_counts[pos] += 1
            else:
    ...
    df_wide = pd.DataFrame(wide_rows, columns=csv_headers)
