...
if gen_btn:
    ...
    st.success(f"Generated {len(filtered_lineups)} lineup(s) within salary range {min_salary}-{max_salary}")
    ...
    if not wide_rows:
        st.error("No lineups match the salary range and position requirements!")
        st.stop()
