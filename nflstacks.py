...
for idx, row in df.iterrows():
    try:
        ...
        # Fix GameStack parsing: store as "TEAM1@TEAM2 DATE TIME"
        if game_info:
            game_info = re.sub(r'[\n\r]', ' ', game_info)
...
if game_stack_size > 0:
    optimizer.add_stack(GameStack(game_stack_size))  # stack X players from the same game
    optimizer.add_stack(GameStack(5, min_from_team=2))  # stack 5 players, at least 2 from one team
...
if gen_btn:
    ...
    if lineups:
        ...
        for lineup in lineups:
            row = {}
            pos_counter = {"QB": 0, "RB": 0, "WR": 0, "TE": 0, "FLEX": 0, "DST": 0}
            for p in lineup.players:
                added = False
                for pos in p.positions:
                    if pos in pos_counter and pos_counter[pos] == 0:
                        row[pos] = f"{player_display_name(p)} ({p.id})"
                        pos_counter[pos] += 1
                        added = True
                        break
                if not added and pos_counter["FLEX"] == 0:
        ...
        st.download_button("Download Lineups CSV", lineup_df.to_csv(index=False).encode("utf-8"), "dfs_lineups.csv", "text/csv")
