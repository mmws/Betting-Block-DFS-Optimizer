...
game_col = find_column(df, ["game","gameinfo","matchup"])
...
site_choice = None
auto_choice = f"{detected_site} {guess_sport_from_positions(df[pos_col])}" if detected_site and guess_sport_from_positions(df[pos_col]) and f"{detected_site} {guess_sport_from_positions(df[pos_col])}" in SITE_MAP else None
if auto_choice:
...
for idx, row in df.iterrows():
    try:
        ...
        if game and '@' in game:
            ...
            game_info = GameInfo(home_team=t1, away_team=t2, game_time=None)  # GameInfo for pydfs
        if salary is None:
            skipped += 1
            continue
