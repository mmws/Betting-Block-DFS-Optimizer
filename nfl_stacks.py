...
if enable_runback:
    runback_positions = st.multiselect(
        "Select positions for runback",
        options=['QB', 'RB', 'WR', 'TE', 'FLEX', 'DST'],
        default=['QB', 'WR']
    )
    num_runback = st.slider(
        "Number of runback players",
        2, 5, 2  # min 2, max 5, default 2
    )
